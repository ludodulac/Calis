  ...
  function finishSession() {
    const log: SessionLog = {
      ...
      exercises: session.exercises.map((exercise) => ({
        ...
        values: values[exercise.resourceSlug].map((value) => Number(value)).filter((value) => Number.isFinite(value) && value > 0),
        completed: values[exercise.resourceSlug].some((value) => Number(value) > 0),
      })),
    };
  ...
  return (
    <div className={styles.sessionScreen}>
      ...
      <footer className={styles.sessionFooter}>
        ...
        <button className={styles.finishButton} type="button" onClick={finishSession}>Terminer</button>
      </footer>
    </div>
  );
}
