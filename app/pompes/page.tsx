...
export default function PushUpsPage() {
  ...
  return (
    <>
      ...
      <section className="section shell"><div className="sectionHeading wide"><div className="eyebrow">Ton chemin</div><h2>Incliner → réussir → stabiliser.</h2><p>La hauteur du support sert de réglage. Le chemin montre le principe sans imposer un nombre de paliers : trouve une version que tu contrôles, rapproche progressivement le mouvement du sol, puis stabilise ta pompe avant de choisir une suite plus difficile.</p></div><div className="progressionPath progressionPathCompact" aria-label="Chemin des pompes inclinées à une pompe stable">{pushUpProgression.map((step,index)=><Link key={step.id} className={`pathStep${step.id === "first-pushup" ? " pathMilestone" : ""}`} href={`/bibliotheque/${step.resourceSlug}`}><span className="pathIndex">{String(index+1).padStart(2,"0")}</span><span className="pathBody"><strong>{step.label}</strong><small>{step.criterion}</small></span><span className="pathArrow" aria-hidden="true">→</span></Link>)}</div><div className="pathFork"><span>Quand ta pompe devient stable</span><div><Link href="/dips">Explorer une autre poussée avec les dips →</Link></div></div></section>
